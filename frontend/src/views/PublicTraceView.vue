...
onMounted(async () => {
  ...
  try { trace.value = await api.get(`/public/tire-traces/${encodeURIComponent(token.value)}`) } catch (cause) { error.value = cause.message }
})
</script>
...
<template>
  <main class="public-trace-page">
    ...
    <section v-else-if="trace" class="trace-document">
      <div class="trace-verdict"><BadgeCheck :size="34" /><div><span>追溯状态</span><strong>{{ trace.tireStatus || trace.traceStatus || '有效' }}</strong></div></div>
      <dl><div v-for="(value, key) in trace" :key="key"><dt>{{ key }}</dt><dd>{{ typeof value === 'object' ? JSON.stringify(value) : value || '-' }}</dd></div></dl>
