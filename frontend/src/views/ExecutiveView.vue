...
const error = ref('')
async function load() { try { data.value = await api.get('/dashboard/executive') } catch (cause) { error.value = cause.message } }
onMounted(load)
</script>
...
<template>
  <main class="workspace-page executive-page">
    ...
    <section class="executive-grid">
      <div class="executive-table"><header><span>现场生产</span><h2>产线运行</h2></header><table><thead><tr><th>产线</th><th>状态</th><th>在制工单</th><th>计划/完成</th><th>故障设备</th></tr></thead><tbody><tr v-for="line in data.productionLines" :key="line.lineId"><td>{{ line.lineName }}</td><td>{{ line.lineStatus }}</td><td>{{ line.activeWorkOrders }}</td><td>{{ line.plannedQty }} / {{ line.actualQty }}</td><td>{{ line.equipmentFaults }}</td></tr></tbody></table></div>
      <div class="alert-feed"><header><span>经营风险</span><h2>异常提醒</h2></header><article v-for="alert in data.alerts" :key="`${alert.domain}-${alert.title}`"><strong>{{ alert.title }}</strong><p>{{ alert.detail }}</p><span>{{ alert.domain }} · {{ alert.severity }}</span></article><p v-if="!data.alerts.length" class="empty-message">暂无经营风险</p></div>
    </section>
